...
public class SummaryStatistics {
    ...
    public double companyStatsWithStreamAverage() {
        setup();
        List<Company> listoOfCompanies = companies;
        //Stream resultLis = (Stream) listoOfCompanies.stream().collect(Collectors.groupingBy(Company::getRevenue));

        double averageNumber = listoOfCompanies.stream().mapToDouble(Company::getRevenue).average().getAsDouble();

        System.out.printf("Average: %.2f%n", averageNumber);

        return averageNumber; // return the average
    }
